...
class UnboundedKnapsack {
    public static int unboundedKnapsackRec(int[] weights, int[] values, int n, int capacity, int[][] dp){
        // Base case
        if (n == 0)
        ...
        if (weights[n] <= capacity){
            taken = values[n] + unboundedKnapsackRec(weights, values, n, capacity - weights[n], dp);
            notTaken = 0 + unboundedKnapsackRec(weights, values, n - 1, capacity, dp);

            // As we want to maximize the profit, we take the maximum of the two values
            dp[n][capacity] = Math.max(taken, notTaken);
        }
        // If weight of the nth item is greater than the capacity
        // Don't include the item
        else
            dp[n][capacity] = unboundedKnapsackRec(weights, values, n - 1, capacity, dp);
        return dp[n][capacity];
    }
    ...
    public static int unboundedKnapsack(int[] weights, int[] values, int n, int capacity){
        ...
        return unboundedKnapsackRec(weights, values, n - 1, capacity, dp);
    }
